      ...
      <div className="grid gap-6">
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            <Button className="sm:col-span-2 w-fit" variant="hero" onClick={save}>Save changes</Button>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5" />My Enrolled Events</CardTitle></CardHeader>
          <CardContent>
            {enrollments.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                You haven't enrolled in any events yet. <Link to="/events" className="text-primary underline">Browse events</Link>.
              </p>
            ) : (
                ...
                {enrollments.map((e) => (
                  <div key={e.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">Enrolled {new Date(e.created_at).toLocaleDateString()} • {e.payment_method}</div>
                    </div>
        ...
        <Card>
          ...
          <CardContent>
            {payments.length === 0 ? (
              <p className="text-sm text-muted-foreground">No payments yet. Payments from license applications and event enrollments will appear here.</p>
            ) : (
